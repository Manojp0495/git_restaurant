...
const Table = ({new_table_number,settablenumber}) => {
...
<center className='mt-2'>
 ...
 {numbers.map((num,index)=>(
  <div style={{display:'inline'}} key={index}>
    <button className={new_table_number===num?activeBtn:inActiveBtn} onClick={()=>settablenumber(num)}>
      {num}
    </button>
  </div>
 ))}
...
const mapStateToProps=(state)=>({
  new_table_number:state.tablereducer.table_number
})
